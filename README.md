...
8. At this point, in order to start the server and bluetooth board processes, I recommend having to terminals open and running the following commands from the first terminal:
  gcc server.c -o server -lpthread
  ./server
and from the second:
  python inoutboard.py
However, I've included short bash scripts to start and end the respective processes, though I'm quite new to scripting so I wouldn't recommend using these. If you do, simply run bash startserver.sh to start and bash endserver.sh to finish; but be cautious, the first script runs the server in the background, so there's a possibility, however slight, that it won't be so easy to actually kill the server.
